...
const TestimonialContainer = ({
  ...
  data: testimonialDataInterface[];
}) => {
  return (
    <>
      {data.concat(data).map((data, index) => (
        <div key={index} className="infinite-carousel-testimonial">
          <div
            className={`relative border-4 ${data.bordercolor} p-2 rounded-2xl z-10 ${data.hoverbordercolor} hover:scale-110 hover:rotate-3 hover:cursor-pointer transition-transform duration-200 ease-in-out`}>
            ...
            <div className="flex p-4 gap-4 items-center">
              <div className="w-16 h-16 bg-green rounded-full">
                <Image
                  src={data.image}
                  alt="testimonial"
                  width={64}
              ...
              <div className="flex flex-col">
                ...
                <div className={`text-xs text-balance ${data.textcolordesig}`}>
                  {data.designation}
                </div>
              </div>
            </div>
